...
save_resized_album_art(image_s *imsrc, const char *path)
{
	...
	{
		if( image_save_to_jpeg_file(imsrc, cache_file) == 0 )
			return cache_file;
		else
			goto error;
	}
	...
	{
		dstw = runtime_vars.album_art_size;
		dsth = (imsrc->height<<8) / ((imsrc->width<<8)/runtime_vars.album_art_size);
	}
	else
	{
		dstw = (imsrc->width<<8) / ((imsrc->height<<8)/runtime_vars.album_art_size);
		dsth = runtime_vars.album_art_size;
	}
#else	// Old Version
	if( imsrc->width > imsrc->height )
	{
